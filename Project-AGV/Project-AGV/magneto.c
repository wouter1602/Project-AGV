...
uint16_t getMagnetoDataX(void) {
	uint16_t data = 0;
	uint8_t dataL = twiRead(MAGNETO_ADDR, OUT_X_L_M);
	uint8_t dataH = twiRead(MAGNETO_ADDR, OUT_X_H_M);
	data = (dataH << 8) | dataL;
	return data;
}
...
uint16_t getMagnetoDataY(void) {
	uint16_t data = 0;
	uint8_t dataL = twiRead(MAGNETO_ADDR, OUT_Y_L_M);
	uint8_t dataH = twiRead(MAGNETO_ADDR, OUT_Y_H_M);
	data = (dataH << 8) | dataL;
	return data;
}