...
int VL53L0X_i2c_init(char *comPortStr, unsigned int baudRate) // mja
{
    ...
    dwWaitResult = WaitForSingleObject(ghMutex, 0xFFFFFFF);
    ...
    if(dwWaitResult == WAIT_OBJECT_0)
    {
        ...
        status = SERIAL_COMMS_Enum_Devices(MAX_DEVICES, devIds,  (uint32_t*)&deviceCount);
        if(status != STATUS_OK)
        {
...
int32_t VL53L0X_platform_wait_us(int32_t wait_us)
{
     ...
     */
    void hEvent = CreateEvent(0, 1, 0, 0);
